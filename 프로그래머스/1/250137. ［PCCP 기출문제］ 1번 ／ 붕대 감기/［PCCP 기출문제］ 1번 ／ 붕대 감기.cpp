...
int solution(vector<int> bandage, int health, vector<vector<int>> attacks) {
    ...
    int last_time = attacks[attacks.size() - 1][0]; // 마지막 공격 시간
    ...
    // 마지막 공격까지 시간 흐름 i는 턴을 의미함
    for (int i = 1; i <= last_time; i++) {
        // 공격 시간과 턴이 같으면
        if (attacks[a_idx][0] == i) {
            now_h -= attacks[a_idx][1]; // 현재 체력에서 공격 데미지 차감
            a_idx++; // 공격 횟수 +1
            h_idx = 0; // 시전 횟수 초기화
            // 현재 체력이 0 이하일 경우 사망
            if (now_h <= 0) {
        ...
        // 공격을 당하지 않았을 경우
            ...
            if (now_h < health) {
                ...
                if (now_h > health) { // 현재 체력이 총 체력보다 높을 경우
                    now_h = health; // 현재 체력을 총 체력과 같게 함
                }
            }
        }
    }
